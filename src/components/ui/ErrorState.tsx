...
export const ErrorState: React.FC<ErrorStateProps> = ({
  ...
  return (
    <div className={`flex flex-col items-center justify-center py-12 px-6 rounded-2xl border border-red-100 bg-red-50/30 ${minHeight} text-center space-y-4 max-w-lg mx-auto w-full`}>
      <div className="p-3 bg-red-100/60 rounded-full text-red-600 border border-red-200 shadow-sm">
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-lg transition-colors border border-transparent shadow-sm hover:shadow active:scale-95"
        >
          Try Again
        </button>
      )}
